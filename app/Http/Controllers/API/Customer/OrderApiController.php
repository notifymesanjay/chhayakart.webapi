...
class OrderApiController extends Controller
{
    public function placeOrder(Request $request)
    {
        ...
        $promo_code = (isset($request->promo_code) && !empty($request->promo_code)) ? $request->promo_code : "";
        $promo_discount = (isset($request->promo_discount) && !empty($request->promo_discount)) ? $request->promo_discount : 0;
        //$active_status = (isset($request->status) && $request->status != "") ? $request->status : 'received';
        $active_status = $payment_method=="COD" ? OrderStatusList::$received: OrderStatusList::$paymentPending;
        ...
        $user_address = CommonHelper::getUserAddress($address_id);
        ...
        /*if (!CommonHelper::isOrderDeliverable($address_id, $latitude, $longitude, $seller_ids[0])) {
        ...
        /* $sql = "INSERT INTO `orders`(`user_id`,`otp`,`mobile`,`order_note`, `total`, `delivery_charge`,
                      `tax_amount`, `tax_percentage`, `wallet_balance`, `promo_code`,`promo_discount`, `final_total`,
                      `payment_method`, `address`, `latitude`, `longitude`, `delivery_time`, `status`, `active_status`,
                      `order_from`,`pincode_id`,`area_id`,`address_id`)
                      VALUES ('$user_id','$otp_number','$mobile','$order_note','$total','$delivery_charge','$order_total_tax_amt',
                              '$order_total_tax_per','$walletvalue','$promo_code','$promo_discount', '$final_total','$payment_method',
                              '$address','$latitude','$longitude','$delivery_time','$order_status','$active_status','$order_from','$pincode_id',
                              '$area_id','$address_id')";
         $db->sql($sql);
        ...
        try {
            ...
            $order->tax_amount = $order_total_tax_amt + $request->tax_amount;
            $order->tax_percentage = $order_total_tax_per + $request->tax_percentage;
    ...
    public function initiateTransaction(Request $request){
        ...
        if ($request->payment_method == "Razorpay") {
            ...
            $order->order_note=  $transaction_id;
            $order->save();
    ...
    public function getOrders(Request $request){
        ...
        foreach ($res as $key => $row) {
            // $res[$key]->address = $row->address." ".$row->landmark." ".$row->area." ".$row->city." ".$row->state."-".$row->pincode." ".$row->country;
             ...
             $res[$key]->address = $row->address." ".$row->landmark." ".$row->area." ".$row->city."-".$row->pincode." ".$row->country;
            ...
            /*$sql = "select oi.*,v.id as variant_id,
                    p.name,p.image,p.manufacturer,p.standard_shipping,p.made_in,p.return_status,p.cancelable_status,p.till_status,
                    v.measurement,(select short_code from unit u where u.id=v.measurement_unit_id) as unit
            from order_items oi
                join product_variant v on oi.product_variant_id=v.id
                join products p on p.id=v.product_id where order_id=" . $row['id'];
            ...
            foreach ($items as $subkey => $item) {
                ...
                if($order_days <= $item->return_days ){
