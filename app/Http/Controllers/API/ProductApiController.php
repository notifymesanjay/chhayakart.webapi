...
class ProductApiController extends Controller
{
    ...
    public function getProducts(Request $request){
        ...
        pv.id as product_variant_id, pv.price, pv.discounted_price,pv.delivery_charges,pv.taxes, pv.status as pv_status , pv.stock,pv.stock_unit_id, u.short_code,
        (select short_code from units where units.id = pv.stock_unit_id) as stock_unit
        ...
        if ($_FILES["file"]["size"] > 0 ) {
            ...
            while (($products = fgetcsv($file, 10000, ",")) !== FALSE) {
                if($count!=0) {
                    ...
                    if (empty($products[9])) {
                        return CommonHelper::responseError('features  is empty at row - ' . $count);
                    }
