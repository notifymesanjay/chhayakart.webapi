...
class CommonHelper
{
    ...
    public static function getSettings($variables){
        ...
        $imageArray = array("favicon","web_logo","loading","logo","popup_image");
        $data = array();
    ...
    public static function validatePromoCode($user_id, $promo_code, $total)
    {
        ...
        if ($code->repeat_usage == 1) {
            ...
            if ($total_usage >= $code->no_of_repeat_usage) {
                $response['is_applicable'] = 0;
    ...
    public static function getSectionWithProduct($seller_ids,$user_id = 0){
        ...
        foreach ($sections as $key => $section){
            ...
            $products = $products->makeHidden(['seller_id','row_order','return_status',
                'cancelable_status','till_status','description','features','status','is_approved','return_days','pincodes',
    ...
    public static function getProductDetails($product_id,$user_id=null,$is_variants=true,$request=null){
        ...
        if($product) {
            $product = $product->makeHidden(['seller_id','row_order','return_status',
                'cancelable_status','till_status','description','features','is_approved','return_days','pincodes',
                           'cod_allowed','pickup_location','tags','d_type','seller_name','seller_slug','seller_status',
    ...
    public static function getProductVariant($variant_id,$user_id=null){
        ...
        if($variant){
            ...
            if ($variant['stock'] <= 0 && $variant['is_unlimited_stock'] == 0) {
                ...
                $variant['status'] = ($variant['status']) ?? 0;
            }
            ...
            $variant['delivery_charges'] = CommonHelper::doubleNumber($variant['delivery_charges']);
            $variant['taxes'] = CommonHelper::doubleNumber($variant['taxes']);
        ...
        if(count($seller_ids)>1) {
            ...
            foreach ($seller_ids as $key => $seller_id) {
                ...
                /*if ($shipping_type == 'local') {
                    $delivery_charge = 0;
                    $dc = $request->delivery_charge == 0 && $delivery_charge > 0 ? $delivery_charge : 0;
                }*/
                $final_total = $totalAmt["total_amount"];
                $final_total = $final_total + $dc; // copan code and $delivery_charge
                ...
                $newOrder->delivery_charge = $order->delivery_charge;
                ...
                $newOrder->promo_discount = $order->promo_discount;
    ...
    public static function getOrderDetails($order_id){
        // $order = Order::select('orders.*','orders.id as order_id','orders.created_at as created_at','users.*','address.name as user_name','users.email as user_email'
        // ,'users.email as user_email','users.mobile as user_mobile','address.*', 'sellers.*', 'sellers.created_at as seller_created','sellers.updated_at as seller_updated','sellers.name as seller_name', 'delivery_boys.name as delivery_boy_name',
        // 'order_items.id as order_item_id','os.id as active_status', 'os.status as status_name', 'cities.id as city_id', 'address.city as city_name')
        ...
        $order = Order::select('orders.*','orders.id as order_id','orders.created_at as created_at','address.name as user_name','users.email as user_email'
            ,'users.email as user_email','users.mobile as user_mobile','address.country as country','address.state as state','address.city as city','address.pincode as pincode','address.area as area','address.landmark as landmark','address.address as address','address.created_at as address_created','address.updated_at as address_updated', 'sellers.created_at as seller_created','sellers.updated_at as seller_updated','sellers.name as seller_name', 'delivery_boys.name as delivery_boy_name',
            'order_items.id as order_item_id','os.id as active_status', 'os.status as status_name', 'cities.id as city_id', 'address.city as city_name')
            ->leftJoin('order_items', 'order_items.order_id', '=', 'orders.id')
            ->leftJoin('users', 'orders.user_id', '=', 'users.id')
            ->leftJoin('user_addresses as address', 'orders.address_id', '=', 'address.id')
        ...
        $order_items = Order::select('order_items.*','orders.mobile','orders.total' ,'orders.delivery_charge','orders.discount','orders.tax_amount','orders.tax_percentage','orders.promo_code',
            'orders.promo_discount','orders.wallet_balance','orders.final_total','orders.payment_method','orders.address','orders.delivery_time',
            'address.name as user_name','order_items.status as order_status','sellers.name as seller_name','products.id as product_id', 'os.id as active_status', 'os.status as status_name')
            ->leftJoin('order_items', 'order_items.order_id', '=', 'orders.id')
            ->leftJoin('users', 'orders.user_id', '=', 'users.id')
            ->leftJoin('user_addresses as address', 'orders.address_id', '=', 'address.id')
            ->leftJoin('product_variants', 'order_items.product_variant_id', '=', 'product_variants.id')
            ...
            if (isset($seller_id) && $seller_id != 0 && $seller_id != "") {
                ...
                if (self::checkOrderMailSendable($seller->admin_id, $order->active_status, 1)) {
                    ...
                    if ($order->active_status == OrderStatusList::$received) {
                        $subject = "Your have " . $status_name . " new order  #" . $order->id;
                    } else {
    ...
    public static function sendNotificationOrderStatus($order){
        ...
        if (isset($seller_id) && $seller_id != 0 && $seller_id != "") {
            ...
            if (self::checkOrderMailSendable($seller->admin_id, $order->active_status, 1)) {
                ...
                if ($order->active_status == OrderStatusList::$received) {
                    $title = "Your have " . $status_name . " new order  #" . $order->id;
                } else {
